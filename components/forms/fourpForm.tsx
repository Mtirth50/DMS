...
export const FourpForm = () => {
  ...
  const fetchEligiblePackets = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/eligible_actipart_packets`, {
  ...
  const handleAssignSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(data.message || "4P assigned successfully");
      setAssignForm({
        packetNo: "",
        kapanNo: "",
        partyName: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      toast.success(data.message || "4P data submitted successfully");
      setSubmitForm({
        packetNo: "",
        kapanNo: "",
        partyName: "",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-lg shadow-lg">
          <CardHeader className="bg-blue-100 text-blue-800 rounded-t-lg p-4">
            <CardTitle className="text-xl font-bold">4P સોંપો</CardTitle>
          </CardHeader>
        ...
        <Card className="rounded-lg shadow-lg">
          <CardHeader className="bg-green-100 text-green-800 rounded-t-lg p-4">
            <CardTitle className="text-xl font-bold">4P સબમિટ કરો</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
